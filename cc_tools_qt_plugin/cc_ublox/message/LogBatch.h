...
class LogBatchImpl;
class LogBatch : public cc_tools_qt_plugin::cc_ublox::Message
{
public:
    LogBatch();
    LogBatch(const LogBatch&) = delete;
    LogBatch(LogBatch&&) = delete;
    virtual ~LogBatch();
    LogBatch& operator=(const LogBatch& other);
    LogBatch& operator=(LogBatch&&);
    static MsgIdParamType doGetId();
    static constexpr bool hasStaticMsgId() { return true; }
    static constexpr MsgIdType staticMsgId() { return static_cast<MsgIdType>(8465U); }
...
private:
    std::unique_ptr<LogBatchImpl> m_pImpl;
};
