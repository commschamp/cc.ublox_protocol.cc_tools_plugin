...
class MonSpanImpl;
class MonSpan : public cc_tools_qt_plugin::cc_ublox::Message
{
public:
    MonSpan();
    MonSpan(const MonSpan&) = delete;
    MonSpan(MonSpan&&) = delete;
    virtual ~MonSpan();
    MonSpan& operator=(const MonSpan& other);
    MonSpan& operator=(MonSpan&&);
    static MsgIdParamType doGetId();
    static constexpr bool hasStaticMsgId() { return true; }
    static constexpr MsgIdType staticMsgId() { return static_cast<MsgIdType>(2609U); }
...
private:
    std::unique_ptr<MonSpanImpl> m_pImpl;
};
